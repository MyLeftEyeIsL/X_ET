...
namespace ETHotfix
{
    ...
    {
        protected override async ETTask Run(Room room, Actor_PlayerEnterRoom_Req request, Actor_PlayerEnterRoom_Ack response, Action reply)
        {
            Gamer gamer = room.Get(request.UserID);
            if (gamer == null)
            {
                //创建房间玩家对象
                gamer = GamerFactory.Create(request.PlayerID, request.UserID);
                await gamer.AddComponent<MailBoxComponent>().AddLocation();
                gamer.AddComponent<UnitGateComponent, long>(request.SessionID);
                ...
                Log.Info($"玩家{request.UserID}进入房间");
            }
            else
            {
                //玩家重连
                ...
                gamer.GetComponent<UnitGateComponent>().GateSessionActorId = request.SessionID;
