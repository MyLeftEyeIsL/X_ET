...
namespace ETHotfix
{
    ...
    {
        protected override async ETTask Run(Gamer gamer, Actor_PlayerExitRoom_Req request, Actor_PlayerExitRoom_Ack response, Action reply)
        {
            ...
            if (room.State == RoomState.Game)
            {
                ...
                Log.Info($"玩家{request.UserID}断开，切换为自动模式");
            }
            else
            {
                //房间移除玩家
